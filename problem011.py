if __name__ == '__main__':
    ...
    consecutive = 4
    candidates = []
    cutoff = consecutive*40
    for i in range(dimension-consecutive):
        for j in range(dimension):
            # Vertical
            digit_sum = 0
            for k in range(consecutive):
                digit_sum += grid[i+k][j]
            if digit_sum>=cutoff:
                candidates.append([i,j,1])
            # Horizontal
            digit_sum = 0
            for k in range(consecutive):
                digit_sum += grid[j][i+k]
            if digit_sum>=cutoff:
                candidates.append([i,j,2])
        for j in range(dimension-consecutive):
            # Diagonal 1
            digit_sum = 0
            for k in range(consecutive):
                digit_sum += grid[i+k][j+k]
            if digit_sum>=cutoff:
                candidates.append([i,j,3])
            # Diagonal 2
            digit_sum = 0
            for k in range(consecutive):
                digit_sum += grid[i+consecutive-k][j+k]
            if digit_sum>=cutoff:
                candidates.append([i,j,4])

    biggest_product = 0
    biggest_product_index = [0,0,0]
    for bunch in candidates:
        digit_product = 1
        i = bunch[0]
        j = bunch[1]
        kind = bunch[2]
        for k in range(consecutive):
            if kind==1:
                digit_product = digit_product * grid[i+k][j]
            if kind==2:
                digit_product = digit_product * grid[j][i+k]
            if kind==3:
                digit_product = digit_product * grid[i+k][j+k]
            if kind==4:
                digit_product = digit_product * grid[i+consecutive-k][j+k]

        if digit_product>biggest_product:
            biggest_product = digit_product
            biggest_product_index = [i,j,kind]

    print(biggest_product_index,"=>",biggest_product)
